...
class TsBlock:
    ...
    def get_extremes(self, start=None, end=None, freq='d'):
        ...
        if temp_extreme:
            ...
            temp_df = get_history_hq(start=start, end=end, freq=freq)
            high = temp_df.high
            low = temp_df.low

        temp_extreme = temp_extreme(high, low)
    ...
    def get_segments(self, start=None, end=None, freq='d'):
        ...
        if temp_segment:
            ...
            temp_segment = temp_segment(higher, lower)
    ...
    def get_blocks(self, start=None, end=None, freq='d'):
        ...
        if temp_block.empty:
            ...
            temp_block = block_identify(higher, lower)
            temp_block = block_relation(temp_block)
    ...
    def get_current_status(self, start=None, end=None, freq='d'):
        ...
        dt = temp_block_df.loc[temp_block_df['No.'] == 0, ['block_flag', 'No.', 'segment_num', 'block_hl_flag']].tail(2).index[0]
        return temp_block_df.loc[dt:]


def block_relation(block_df):
...
def block_identify(higher, lower):
    # 后向寻找Block
...
def segment(higher, lower):
    ...
    # 合并高低点后再处理一次
    return segment(higher, lower)


def extreme(high, low):
    """
